...
import { UserIcon, DeepseekIcon, ChatGPTIcon, GeminiIcon } from './icons';
...
const ContentRenderer: React.FC<{ content: string }> = ({ content }) => {
    ...
    return (
        <div className="text-text-primary whitespace-pre-wrap font-sans">
...
const MessageComponent: React.FC<MessageProps> = ({ message }) => {
  ...
  if (isSystem) {
    return (
      <div className="flex justify-center my-4">
        <p className="text-sm text-text-secondary px-4 py-1 bg-border-theme bg-opacity-75 rounded-full">{text}</p>
      </div>
    );
  }
  ...
  return (
    <div className={`flex items-end gap-3 my-4 ${alignment}`}>
      {!isUser && <Avatar />}
      <div className={`max-w-xs md:max-w-md lg:max-w-2xl px-4 py-3 rounded-2xl ${bubbleColor} ${bubblePosition} shadow-sm`}>
